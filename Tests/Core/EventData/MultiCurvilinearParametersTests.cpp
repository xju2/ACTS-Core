...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(multi_curvilinear_initialization)
  {
    ...
    CurvilinearParameters* curvilinear_pos_0         = new CurvilinearParameters(nullptr, pos0, mom0, 1.);
    CurvilinearParameters* curvilinear_pos_1         = new CurvilinearParameters(nullptr, pos1, mom1, 1.);
    CurvilinearParameters* curvilinear_pos_2         = new CurvilinearParameters(nullptr, pos2, mom2, 1.);
    CurvilinearParameters* curvilinear_neg = new CurvilinearParameters(nullptr, pos0, mom0, -1.);
	...
	//test sort method
	MultipleCurvilinearParameters::TrackParMapConstIter it = multi_curvilinear_pos.getTrackList().begin();
    ...
    MultiConsistencyCheck(
        multi_curvilinear_pos, 0, pos0, mom0, +1., {{0., 0., fphi, ftheta, oOp}});
    ...
    Vector3D uposition = curvilinear_neg->referenceSurface().transform(tgContext)
    ...
    CHECK_CLOSE_REL(
        multi_curvilinear_pos.referenceSurface(0).center(tgContext), uposition, 1e-6);
    ...
    CHECK_CLOSE_REL(
        ...
        multi_curvilinear_pos.referenceSurface(0).transform(tgContext).rotation().col(2),
        1e-6);
    ...
    CurvilinearParameters* curvilinear_pos_update = new CurvilinearParameters(nullptr, pos0, mom0, 1.);
	...
	MultipleCurvilinearParameters multi_curvilinear_pos_update( 0.4,curvilinear_pos_update);
    CurvilinearParameters* curvilinear_pos_update_1   = new CurvilinearParameters(nullptr, pos1, mom1, 1.);
    CurvilinearParameters* curvilinear_pos_update_2   = new CurvilinearParameters(nullptr, pos2, mom2, 1.);
    ...
    CHECK_CLOSE_REL(
        multi_curvilinear_pos_update.referenceSurface().center(tgContext), multi_curvilinear_pos.referenceSurface().center(tgContext), 1e-6);
  }

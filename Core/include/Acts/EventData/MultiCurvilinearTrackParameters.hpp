...
public:
  ...
  MultiCurvilinearTrackParameters(double               weight,
  ...
  template <typename T = ChargePolicy,
            std::enable_if_t<std::is_same<T, ChargedPolicy>::value, int> = 0>
  MultiCurvilinearTrackParameters() : MultiTrackParameters<ChargePolicy>()
  {
  }
  // default constructor
  template <typename T = ChargePolicy,
  ...
  MultiCurvilinearTrackParameters(
    ...
    : MultiTrackParameters<ChargePolicy>(copy), m_upSurface(copy.m_upSurface)
  {
  }

...
namespace detail {
  ...
  /// For two arbitrary probability density function, p1, p2 (Gaussian pdf), the
  /// KL distance should be
  /// Dkl = tr[ (V1-V2)(W1-W2)]+(u1-u2).Transpose()(W1+W2)(u1-u2)
  /// e.g. track model, u1 represents meaning of parameter of track1, V1
  /// represents meaning of covaiance of track1.
  ///
  ...
  struct KullbackLeiblerComponentDistance
  {
    ...
    double
    operator()(const weight_parameter_t& weight_parameters_1,
               const weight_parameter_t& weight_parameters_2) const
    {
      ...
      if (do1D) {
        ...
        double distance = covarianceDifference * G_difference
        ...
        double      distance = (covarianceDifference * G_difference).trace()
            + (parametersDifference.transpose() * G_sum * parametersDifference);
        return distance;
      }
    }
  };

...
namespace detail {
  ...
  struct BetheHitler
  {
    ...
    struct Polynomial
    {
      ...
      {
        double                              sum(0.);
        std::vector<double>::const_iterator coefficient
    ...
    {
      ComponentValues(const double w, const double m, const double c)
        : weight(w), mean(m), variance(c)
      {
      }
      double weight;
      double mean;
               ...
               NavigationDirection navDir) const
    {
      ...
      if (pathLX0 < 0.0001) {
        ...
        double varZ
            = exp(-1. * pathLX0 * log(3.) / log(2.)) - exp(-2. * pathLX0);
        if (navDir == forward) {
        ...
        std::string readFile = "/afs/cern.ch/work/j/jinz/acts-core/Core/"
                               "include/Acts/Extrapolator/detail/"
        ...
        for (unsigned int componentIndex = 0; componentIndex < N;
             ++componentIndex) {
        ...
        for (unsigned int componentIndex = 0; componentIndex < N;
          ...
          if (navDir == forward) {
            ...
            deltaVar = 1. / ((momentum * updatedMean) * (momentum * updatedMean)
                             * updatedCov);
          } else {
    ...
    Polynomial
    readPolynomial(std::ifstream& fin, const int order) const
    {
  ...
  std::ostream&
  operator<<(std::ostream& os, const BetheHitler::ComponentValues& obj)
  {
    os << "weight mean cov " << obj.weight << "," << obj.mean << ","
       << obj.variance << " ";
    return os;
  }
}

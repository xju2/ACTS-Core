...
Acts::MultiEigenStepper<B, C, E, A>::momentum(const State& state) const
{
  ...
  for (const auto& tuple_state : state.stateCol) {
    if (std::get<2>(tuple_state) != StateStatus::FREE) continue;
    double component_mom = EigenStepper<B>::momentum(std::get<0>(tuple_state));
    ...
    const Surface* surface) const
{
  ...
  for (auto& tuple_state : state.stateCol) {
    ...
    if (!surface->isOnSurface(state.geoContext,
                              EigenStepper<B>::position(singlestate),
...
Acts::MultiEigenStepper<B, C, E, A>::outPut(const State& state) const
{
  for (const auto& tuple_state : state.stateCol) {
    ...
    std::cout << "the single component stat: " << stat << " weight " << weight
              << std::endl;
                                                ...
                                                bool             release) const
{
  for (auto& tuple_state : state.stateCol) {
    ...
    if (std::get<2>(tuple_state) != StateStatus::FREE) continue;
    auto& singlestate = std::get<0>(tuple_state);
    EigenStepper<B>::updateStep(singlestate, navCorr, navigationStep, release);
  }
}
                                                ...
                                                cstep::Type type) const
{
  for (auto& tuple_state : state.stateCol) {
    ...
    if (std::get<2>(tuple_state) != StateStatus::FREE) continue;
    auto& singlestate = std::get<0>(tuple_state);
    EigenStepper<B>::updateStep(singlestate, abortStep, type);
  }
}

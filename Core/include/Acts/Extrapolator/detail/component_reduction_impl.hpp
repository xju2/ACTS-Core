...
namespace impl {
    ...
    while (numberOfComponents > constraintNum) {
      ...
      while (numberOfComponents > constraintNum && !unmergedMap.empty()) {
        if (unmergedMap.size() > 1) {
          ...
          auto combinedComponent   = combiner(
              geoContext, surface, *trackMap.begin(), *mergedComponentIter);
          unmergedMap.erase(mergedComponentIter);
          unmergedMap.erase(trackMap.begin());
          mergedMap.insert(std::make_pair(
              combinedComponent.first,
              std::unique_ptr<TrackParametersBase>(combinedComponent.second)));
          --numberOfComponents;
        } else {
  ...
  template <typename T>
  auto
  pairWithMinimumDistance(const T& unmergedMap) -> typename T::const_iterator
  {
